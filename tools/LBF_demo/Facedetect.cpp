...
int FaceDetectionAndAlignment(const char* inputname,string modelPath){
    ...
    regressor.Load(modelPath+"LBF.model",modelPath+"Regressor.model");
...
void detectAndDraw( Mat& img, CascadeClassifier& cascade,
    ...
    for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ ){
		//Rect rect = cv::Rect(r->x + 0.13*r->width, r->y + 0.25*r->height, r->width *0.35, r->height *0.35);
        //Point center;
       ...
       /* boundingbox.start_x = rect.x*scale;
        boundingbox.start_y = rect.y*scale;
        boundingbox.width   = (rect.width-1)*scale;
        boundingbox.height  = (rect.height-1)*scale;
        boundingbox.centroid_x = boundingbox.start_x + boundingbox.width/2.0;
        boundingbox.centroid_y = boundingbox.start_y + boundingbox.height/2.0;*/
...
//                  cvPoint(boundingbox.start_x+boundingbox.width,boundingbox.start_y+boundingbox.height),Scalar(0,255,0), 1, 8, 0);
    ...
    if(a=='s'){
        save_count++;
        imwrite(std::to_string(save_count)+".jpg", img);
    }
}
