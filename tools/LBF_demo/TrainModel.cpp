...
void TrainModel(vector<string> trainDataName
	,string modelPath,string dataPath,string cascadeName){
    ...
    for(int i=0;i<trainDataName.size();i++){
       ...
       // LoadData(path, images, ground_truth_shapes, bounding_boxs);
          LoadOpencvBbxData(path, images, ground_truth_shapes, bounding_boxs,cascadeName);
    }

    LBFRegressor regressor;
    regressor.Train(images,ground_truth_shapes,bounding_boxs);
    regressor.Save(modelPath+"LBF.model",modelPath+"Regressor.model");
    return;
}
